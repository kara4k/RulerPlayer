...
public class CardFragment extends MusicFragment
    ...
    private void navigateHomeDir() {
        String homeFolder = Preferences.getHomeFolder(getContext());
        File homeDirPath = new File(homeFolder);
        if (homeDirPath.exists()) {
            updateUI(homeDirPath);
        } else {
    ...
    private void setCurrentDir() {
        ...
        if (mCurrentTrack != null && !mCurrentTrack.isOnline() && mCurrentTrack.getFile().exists()) {
            mCurrentDir = mCurrentTrack.getFile().getParentFile();
        } else {
            String homeDirPath = Preferences.getHomeFolder(getContext());
